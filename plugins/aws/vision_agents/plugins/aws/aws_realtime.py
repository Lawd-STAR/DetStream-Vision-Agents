    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.output_track = AudioStreamTrack(
            framerate=24000, stereo=False, format=AudioFormat.S16
        )
                        ...
                        try:
                            ...
                            if "event" in json_data:
                                if "contentStart" in json_data["event"]:
                                    ...
                                    logger.info(
                                        f"Tool use from AWS Bedrock: {json_data['event']['toolUse']}"
                                    )
                                    ...
                                    logger.info(
                                        f"Tool end from AWS Bedrock: {json_data['event']['contentEnd']}"
                                    )
                                    ...
                                    stopReason = json_data["event"]["contentEnd"][
                                        "stopReason"
                                    ]
                                    if stopReason == "INTERRUPTED":
                                        logger.info("TODO: should flush audio buffer")
                                    logger.info(
                                        f"Content end from AWS Bedrock {stopReason}: {json_data['event']['contentEnd']}"
                                    )
