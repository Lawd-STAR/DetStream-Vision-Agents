...
from stream_agents.llm.openai_realtime_llm import OpenAIRealtimeLLM
...
class TestOpenAIRealtimeLLM:
    ...
    async def test_simple(self, llm: OpenAIRealtimeLLM):
        ...
        @llm.on("connected")
        async def on_connected(event: STSConnectedEvent):
            nonlocal connected
            connected = True
    ...
    async def test_event_emission(self, llm: OpenAIRealtimeLLM):
        ...
        try:
            ...
            connected_events = [e for e in events if isinstance(e, STSConnectedEvent)]
            ...
            response_events = [e for e in events if isinstance(e, STSResponseEvent)]
    ...
    async def test_error_handling(self):
        ...
        @llm.on("error")
        async def on_error(event: STSErrorEvent):
