...
class TavusDailyEventHandler(EventHandler):
    ...
    def on_track_started(self, data, error):
        ...
        if data.get('track', {}).get('kind') == 'audio':
            ...
            self.logger.info(f"🎵 Audio track started from participant {participant_id}")
            self.logger.info(f"🎵 Audio track details: {data.get('track')}")
            # Start forwarding audio frames
            asyncio.create_task(self.processor._forward_audio_from_daily(participant_id))
        
        # Handle video tracks  
        elif data.get('track', {}).get('kind') == 'video':
            participant_id = data.get('participant', {}).get('id')
            self.logger.info(f"🎥 Video track started from participant {participant_id}")
            self.logger.info(f"🎥 Video track details: {data.get('track')}")
            # Start forwarding video frames
            asyncio.create_task(self.processor._forward_video_from_daily(participant_id))
...
class DailyAudioTrack(AudioStreamTrack):
    ...
    async def recv(self):
        ...
        try:
            ...
            self.logger.debug(f"🔇 Returning silent audio frame: {frame.sample_rate}Hz, {len(samples)} samples")
            return frame
        except Exception as e:
...
class TavusClient:
    ...
    def create_conversation(
        ...
        **kwargs
        ...
        try:
            self.logger.info(f"Creating Tavus conversation with replica_id={replica_id}, persona_id={persona_id}")
            ...
            self.logger.info(f"Successfully created conversation: {conversation_data.get('conversation_id')}")
            ...
            if hasattr(e, 'response') and e.response is not None:
                ...
                try:
                    ...
                    self.logger.error(f"Could not parse error response as JSON: {json_error}")
    ...
    def end_conversation(self, conversation_id: str) -> Dict[str, Any]:
        ...
        """
        try:
            response = self.session.post(f"{self.base_url}/conversations/{conversation_id}/end")
            response.raise_for_status()
            return response.json()
        except requests.RequestException as e:
...
class TavusProcessor(AudioVideoProcessor, AudioPublisherMixin, VideoPublisherMixin):
        ...
        if auto_create:
            try:
                ...
                if auto_join and self.conversation_url:
                    self.logger.info(f"🔗 Auto-joining Daily call: {self.conversation_url}")
    ...
    async def _join_daily_call(self):
        ...
        if not self.conversation_url or self._call_joined:
            self.logger.info(f"⚠️  Skipping Daily join - URL: {self.conversation_url}, already joined: {self._call_joined}")
            return
        ...
        try:
            ...
            def join_completion(data, error):
                self.logger.info(f"🔔 Join completion callback called - data: {data}, error: {error}")
                if error:
                    self.logger.error(f"❌ Failed to join Daily call: {error}")
                else:
            ...
            self.daily_client.join(
                meeting_url=self.conversation_url,
            ...
            for i in range(10):  # Wait up to 10 seconds
                ...
                self.logger.info(f"📊 Waiting for connection... ({i+1}/10) - _call_joined: {self._call_joined}")
                if self._call_joined:
                    break
            
            if not self._call_joined:
                self.logger.warning("⚠️  Daily call join may not have completed successfully")
            ...
            try:
                ...
                for participant_id, participant_data in participants.items():
                    if participant_id != 'local':  # Skip our local participant
                        ...
                        if 'tavus' in user_info.get('userId', '').lower() or 'replica' in user_info.get('userId', '').lower():
                            tavus_participant_id = participant_id
                            self.logger.info(f"🎭 Found Tavus participant: {participant_id}")
                            break
                
                # If we found a Tavus participant, start forwarding their media

                if tavus_participant_id and self._call_joined:
                    self.logger.info(f"🚀 Starting media forwarding from Tavus participant: {tavus_participant_id}")
                    asyncio.create_task(self._forward_video_from_daily(tavus_participant_id))
                    asyncio.create_task(self._forward_audio_from_daily(tavus_participant_id))
    ...
    async def _forward_audio_from_daily(self, participant_id: str):
        ...
        self.logger.info(f"🎵 Starting audio forwarding from participant {participant_id}")
    ...
    async def _forward_video_from_daily(self, participant_id: str):
        ...
        self.logger.info(f"🎥 Starting video forwarding from participant {participant_id}")
    ...
    async def _generate_test_video(self):
        ...
        try:
            while self.daily_video_track and not self.daily_video_track._stopped:
                ...
                test_frame = await self._create_test_video_frame_with_counter(frame_count)
                if test_frame and self.daily_video_track:
                    await self.daily_video_track.add_video_frame(test_frame)
                    if frame_count % 30 == 0:  # Log every second
                        self.logger.info(f"🎬 Generated test video frame #{frame_count}")
