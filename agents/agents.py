...
class TTS(Protocol):
    ...
    def set_output_track(self, track: audio_track.AudioStreamTrack) -> None:
...
class Agent:
    ...
    async def join(self, call, user_creation_callback: Optional[Callable] = None) -> None:
        """
        ...
        try:
            ...
            if "NoneType" in str(e) and "await" in str(e):
                self.logger.warning("Cleanup error (likely WebSocket already closed) - ignoring")
            else:
