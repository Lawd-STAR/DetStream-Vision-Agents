...
from vision_agents.core.agents.conversation import InMemoryConversation, Message, StreamingMessageHandler
...
class StreamStreamingMessageHandler(StreamingMessageHandler):
    ...
    def __init__(self, message_id: str, user_id: str, role: str, conversation: 'StreamConversation' = None):
    ...
    async def _on_content_created(self):
        ...
        if self.conversation and self.conversation.channel:
            try:
                ...
                self.conversation.internal_ids_to_stream_ids[self.message_id] = self.stream_message_id
                
                logger.debug(f"Created Stream message {self.stream_message_id} for internal message {self.message_id}")
            except Exception as e:
                logger.error(f"Failed to create Stream message for {self.message_id}: {e}")
                raise
    ...
    async def _on_content_changed(self):
        ...
        if self.conversation and self.conversation.channel and self.stream_message_id:
            print("performing ephemeral_message_update on stream API now")
            try:
                ...
                logger.debug(f"Updated Stream message {self.stream_message_id} with content: {self.content[:50]}...")
            except Exception as e:
                logger.error(f"Failed to update Stream message {self.stream_message_id}: {e}")
                raise
    ...
    async def _on_finalized(self):
        ...
        if self.conversation and self.conversation.channel and self.stream_message_id:
            try:
                ...
                logger.error(f"Failed to finalize Stream message {self.stream_message_id}: {e}")
                raise
...
class StreamConversation(InMemoryConversation):
    ...
    def _create_handler(self, message_id: str, user_id: str, role: str) -> StreamStreamingMessageHandler:
    ...
    async def add_message(self, message: Message, completed: bool = True):
        ...
        response = await self.channel.send_message(
            request
        )
