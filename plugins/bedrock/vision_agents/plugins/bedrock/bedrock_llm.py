...
class BedrockLLM(LLM):
    ...
    async def converse_stream(self, *args, **kwargs) -> LLMResponseEvent[Any]:
        ...
        try:
            ...
            self.events.send(LLMResponseCompletedEvent(original=last_event, text=total_text, plugin_name="bedrock"))
    ...
    def _process_stream_event(
        ...
        """Process a streaming event from Bedrock."""
    ...
    @staticmethod
    def _normalize_message(bedrock_messages: Any) -> List["Message"]:
        """Normalize Bedrock messages to internal Message format."""
        from vision_agents.core.agents.conversation import Message

        if isinstance(bedrock_messages, str):
            bedrock_messages = [
                {"content": [{"text": bedrock_messages}], "role": "user"}
            ]

        if not isinstance(bedrock_messages, (List, tuple)):
