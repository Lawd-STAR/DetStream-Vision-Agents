...
def calculate_stt_metrics(events: List[BaseEvent]) -> Dict[str, Any]:
    ...
    if processing_times:
        metrics.update(
            {
                "avg_processing_time_ms": math.fsum(processing_times)
...
def calculate_tts_metrics(events: Sequence[BaseEvent]) -> Dict[str, Any]:
    ...
    if completion_events:
        ...
        metrics.update(
            {
                "avg_synthesis_time_ms": math.fsum(synthesis_times)
                / len(synthesis_times),
        ...
        if real_time_factors:
            metrics.update(
                {
                    "avg_real_time_factor": math.fsum(real_time_factors)
...
def calculate_vad_metrics(events: Sequence[BaseEvent]) -> Dict[str, Any]:
    ...
    if audio_events:
        ...
        if durations:
            metrics.update(
                {
                    "avg_speech_duration_ms": math.fsum(durations) / len(durations),
                    "total_speech_duration_ms": math.fsum(durations),
                }
            )
        ...
        if probabilities:
            metrics.update(
                {
                    "avg_speech_probability": math.fsum(probabilities)
                    / len(probabilities),
                }
            )
