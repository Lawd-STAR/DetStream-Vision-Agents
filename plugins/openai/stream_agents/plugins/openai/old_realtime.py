...
class RealtimeConnection:
    ...
    def __init__(
        ...
        *,
        ...
        self._switch_video_track: MediaStreamTrack = None
    ...
    async def _start_session(self):
        ...
        try:
            ...
            if self.enable_video_input:
                ...
                class SwitchingVideoTrack(_VideoStreamTrack):
                    kind = "video"

                    def __init__(self, width: int, height: int, fps: int, outer_conn: "RealtimeConnection"):
                        super().__init__()
                        self._width = int(width)
                        self._height = int(height)
                        self._fps_limit = max(1, int(fps))
                        self._last_tick_ts = None
                        self._source: Optional[MediaStreamTrack] = None
                        self._outer = outer_conn

                    def set_source(self, source: Optional[MediaStreamTrack]) -> None:
                        self._source = source
                    ...
                    async def _next_tick(self) -> None:
                        ...
                        if self._last_tick_ts is not None:
                            remaining = (self._last_tick_ts + interval) - now
                            if remaining > 0:
                                await asyncio.sleep(remaining)
                                now = _time.monotonic()
                        self._last_tick_ts = now

                    async def _generate_black_frame(self) -> VideoFrame:
                    ...
                    async def recv(self) -> VideoFrame:
                        ...
                        source = self._source
                        if source is None:
                            return await self._generate_black_frame()

                        # Drain freshest available frame with tiny timeouts, else wait for next
                        frame: Optional[VideoFrame] = None
                        while True:
                            try:
                                f = await asyncio.wait_for(source.recv(), timeout=0.001)
                                frame = f
                                continue
                            except asyncio.TimeoutError:
                                break
                            except MediaStreamError:
                                frame = await self._generate_black_frame()
                                break
                            except Exception:
                                break
                        if frame is None:
                        ...
                        # Optional diagnostics/snapshots
                        if self._outer._video_debug_enabled or self._outer._save_snapshots_enabled:
                            try:
                                arr = frame.to_ndarray(format="rgb24")
                                self._outer._last_frame_rgb = arr
                                self._outer._last_frame_ts = asyncio.get_event_loop().time()
                                if self._outer._video_debug_enabled and not getattr(self, "_debugged", False):
                ...
                try:
                    self._switch_video_track = SwitchingVideoTrack(
                        width=self._target_video_width,
                        height=self._target_video_height,
                        fps=self._video_fps,
                        outer_conn=self,
                    )
                    # Ensure a video m-line by adding the switchable track now (black until source set)
                    self._video_sender = self.pc.addTrack(self._switch_video_track)
                    logger.info(
                        f"Negotiated sendonly video via addTrack: sender={'yes' if self._video_sender else 'no'}"
                    )
                except Exception:
                    logger.warning("Failed to negotiate switchable video track; video input disabled")
                    self._switch_video_track = None
                    self._video_sender = None
    ...
    async def start_video_sender(self, source_track: MediaStreamTrack, fps: Optional[int] = None) -> None:
        ...
        try:
            ...
            # Ensure the negotiated switchable track exists
            if self._switch_video_track is None:
                raise RuntimeError("Switchable video track not negotiated")

            # Optionally update fps limit on the switchable track
            _fps = max(1, int(fps)) if fps is not None else self._video_fps
            try:
                set_fps = getattr(self._switch_video_track, "set_fps", None)
                if callable(set_fps):
                    set_fps(_fps)
            except Exception:
                pass

            # Attach source; no replaceTrack needed

            self._switch_video_track.set_source(source_track)

            # Apply encoding constraints to honor maxFramerate
            video_sender = self._active_video_sender or self._video_sender
            if video_sender is not None:
                await self._apply_video_sender_constraints(video_sender)
            logger.warning(
                "start_video_sender: forwarding via switchable track fps=%s source_kind=%s source_cls=%s",
                _fps,
                getattr(source_track, "kind", None),
                source_track.__class__.__name__,
            )
            # Ensure SDP reflects newly active video by renegotiating
    ...
    async def stop_video_sender(self) -> None:
        ...
        try:
            if self._switch_video_track is not None:
                set_source = getattr(self._switch_video_track, "set_source", None)
                if callable(set_source):
                    set_source(None)
                logger.info("Video sender reverted to black frames on switchable track")
            # Renegotiate to reflect that source is now inactive
            await self._renegotiate()
        except Exception as e:
            logger.error(f"Failed to stop video sender: {e}")

    async def _renegotiate(self) -> None:
        """Perform a fresh offer/answer exchange to reflect current tracks in SDP."""
        try:
            offer = await self.pc.createOffer()
            await self.pc.setLocalDescription(offer)
            answer_sdp = await self._exchange_sdp(offer.sdp)
            if not answer_sdp:
                raise RuntimeError("Failed to exchange SDP during renegotiation")
            answer = RTCSessionDescription(sdp=answer_sdp, type="answer")
            await self.pc.setRemoteDescription(answer)
            logger.info("Renegotiation completed")
        except Exception as e:
...
class Realtime(realtime.Realtime):
    ...
    def __init__(
        ...
        *,
        ...
        # If True, do not connect to provider until a video source is available
        self.delay_until_video: bool = bool(delay_until_video)

        # Auto-start only when not delaying until video
        if not self.delay_until_video:
            try:
    ...
    async def _ensure_connection(self):
        ...
        async with self._connection_lock:
            if self.delay_until_video and not getattr(self, "_video_source_pending", False):
                return
    ...
    async def start_video_sender(self, track: MediaStreamTrack, fps: Optional[int] = None) -> None:
        ...
        # If delaying until video, mark source pending and create connection now
        if self.delay_until_video and not self._is_connected:
            self._video_source_pending = True
            await self._ensure_connection()
